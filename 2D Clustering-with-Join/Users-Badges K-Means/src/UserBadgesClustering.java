...
public class UserBadgesClustering {
  ...
  public static void runJob(String[] input, String output) throws Exception {
    ...
    while(!converged){
      ...
      job.setJarByClass(UserBadgesClustering.class);
      job.setMapperClass(UserBadgesClusteringMapper.class);
      job.setReducerClass(UserBadgesClusteringReducer.class);
      ...
      if(iteration == 0){
         ...
         conf.set("C1", "0.10\t0.10");      //Initial centroids (3) passed to Mapper via set().
         conf.set("C2", "0.45\t0.80");
         conf.set("C3", "0.80\t0.10");
         oldCentroids.add(new DoubleDoublePair(0.10,0.10));    //Initial centroids (3) added to ArrayList
         oldCentroids.add(new DoubleDoublePair(0.45,0.80));    //in order to compare with new centroids later on.
         oldCentroids.add(new DoubleDoublePair(0.80,0.10));
       ...
       ArrayList<DoubleDoublePair> newCentroids = new ArrayList<DoubleDoublePair>();
  ...
  public static void clusterDataPoints(String[] input, String output, int iteration)
       ...
       job.setJarByClass(UserBadgesClustering.class);
       job.setMapperClass(UserBadgesClusteringMapper.class);
       job.setReducerClass(UserBadgesDataPointsReducer.class);
       job.setMapOutputKeyClass(DoubleDoublePair.class);
       job.setMapOutputValueClass(DoubleDoublePair.class);
       job.setOutputKeyClass(DoubleDoublePair.class);
